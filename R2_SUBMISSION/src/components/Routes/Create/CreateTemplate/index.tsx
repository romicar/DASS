...
function TemplateInfo({ setTemplateInfoData, templateInfoData, active, setActive, setVisible, setText }:
    {
        setTemplateInfoData: React.Dispatch<React.SetStateAction<TemplateInfoInterface>>,
        templateInfoData: TemplateInfoInterface,
        active: Boolean,
...
function CreatePage() {
    ...
    const saveTemplate = async () => {
        ...
        try {
            let ret = await base('Templates').create([
                {
            ...
            if (savedTemplates.length === 0) {
                setIsLoading(false);
                setVisible(true);
                setText("Template created successfully");
                setTimeout(() => {
                    navigate(`/templates/EditTemplate/${id_}`)
                }, 2000)
            }
            ...
            const xx = getChunks(SubTemplates);
    ...
    const [temsave, setTemSave] = useState(true);
        ...
        <Page>
            ...
            <Section>
                ...
                <span style={{ float: "right" }}>
                    <TransButton
                        onClick={() => {
                            if (window.confirm("Confirm Reload") === true) {
                                ...
                                {(provided: any) => (
                                    <div className="characters" {...provided.droppableProps} ref={provided.innerRef}>
                                        {snippetData.map((data, index) => {
                                            if (data.type === 'silence') {
                                                return (
                                                    <div className="dragg"><Draggable key={data.id} draggableId={data.id} index={index} >
                                            ...
                                            return (
                                                <div className="dragg"><Draggable key={data.id} draggableId={data.id} index={index} className="dragg">
                                                    {(provided: any) => (
                                                        <SectionContent ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                                                            <SnippetContainer
